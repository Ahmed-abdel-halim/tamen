...
type CashInTransitInsuranceDocument = {
  ...
  created_at: string;
  insured_name: string;
  limit_per_transit: number | string;
...
export default function CashInTransitInsuranceList({ isArchive = false }: { isArchive?: boolean } = {}) {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/cash-in-transit-insurance?${params.toString()}`, { headers });
      const data = await res.json();
              ...
              <tbody>
                {paginatedDocuments.map((doc) => (
                  <tr key={doc.id}>
                    ...
                    <td>{parseFloat(String(doc.limit_per_transit)).toFixed(3)} د.ل</td>
                    ...
                    <td>
                      <div className="action-buttons">
                        <button
                          onClick={() => {
                            window.open(`${API_BASE_URL}/cash-in-transit-insurance/${doc.id}/print?t=${Date.now()}`, '_blank');
                          }}
