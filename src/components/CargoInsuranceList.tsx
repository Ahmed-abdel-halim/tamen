...
export default function CargoInsuranceList({ isArchive = false }: { isArchive?: boolean } = {}) {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/cargo-insurance?${params.toString()}`, { headers });
      const data = await res.json();
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      try {
        ...
        let total = 0;
        let lastPage = 1;
        const fetchedDocs: typeof allDocs = [];
      ...
      const columns = [
        ...
        { header: 'نوع الشحن', key: 'transport_type', width: 20 },
        { header: 'مبلغ التأمين', key: 'sum_insured', width: 20 },
      ...
      const data = allDocs.map(doc => ({
        ...
        sum_insured: parseFloat(String(doc.sum_insured)).toLocaleString() + ' د.ل',
  ...
  return (
    <section className="users-management">
                  ...
                  <tr key={doc.id}>
                    ...
                    <td>
                      <div className="action-buttons">
                        <button
                          onClick={() => {
                            window.open(`${API_BASE_URL}/cargo-insurance/${doc.id}/print?t=${Date.now()}`, '_blank');
                          }}
      ...
      <InsuranceTermsModal
        ...
        insuranceTypeKey="cargo"
        insuranceTypeName="تأمين شحن البضائع"
      />
          {cancellationDoc && (
        <CancelDocumentModal
