...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    body { font-family: 'Cairo','Tahoma',sans-serif; direction: rtl; background:#fff; color:#0f172a; width:210mm; min-height:297mm; margin:0 auto; }
    .watermark { position:fixed; top:50%; left:50%; transform:translate(-50%,-50%) rotate(-35deg); font-size:90px; font-weight:900; color:rgba(1,76,177,0.04); white-space:nowrap; pointer-events:none; z-index:0; user-select:none; }
    .top-stripe,.bottom-stripe { height:8px; background:linear-gradient(90deg,#014cb1 0%,#0ea5e9 50%,#014cb1 100%); }
    ...
    .num-badge { margin-top:6px; background:#014cb1; color:#fff; font-size:13px; font-weight:900; padding:5px 22px; border-radius:20px; display:inline-block; }
    .divider { height:3px; background:linear-gradient(90deg,transparent,#014cb1 20%,#22d3ee 50%,#014cb1 80%,transparent); border-radius:4px; margin:12px 0; }
    ...
    .row-full { display:flex; align-items:center; background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:10px 14px; gap:14px; margin-bottom:10px; }
    .row-label { font-size:12px; font-weight:800; color:#1e3a8a; white-space:nowrap; min-width:150px; border-left:2.5px solid #014cb1; padding-left:12px; }
    ...
    .amount-box { display:flex; align-items:center; background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%); border:2.5px solid #014cb1; border-radius:12px; padding:18px 20px; gap:16px; margin-bottom:10px; position:relative; overflow:hidden; }
    ...
    .amount-currency { font-size:13px; font-weight:800; color:#334155; background:#fff; border:1px solid #e2e8f0; border-radius:6px; padding:4px 12px; }
    .amount-only { font-size:10px; font-weight:700; color:#94a3b8; border:1px dashed #cbd5e1; border-radius:4px; padding:2px 8px; }
    ...
    .stamp-circle { width:80px; height:80px; border:2.5px dashed #94a3b8; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:10px; color:#94a3b8; font-weight:700; text-align:center; line-height:1.4; }
    .footer { display:flex; justify-content:space-between; flex-wrap:wrap; gap:8px; padding:10px 0 0; font-size:9.5px; color:#64748b; font-weight:600; border-top:1px solid #e2e8f0; margin-top:14px; }
    .print-btn { display:block; margin:20px auto; padding:12px 40px; background:#014cb1; color:#fff; border:none; border-radius:8px; font-family:'Cairo',sans-serif; font-size:16px; font-weight:800; cursor:pointer; }
...
<body>
  ...
  <div class="content">
    ...
    <div class="info-grid">
      <div class="info-cell"><div class="info-label">📅 تاريخ القبض</div><div class="info-value">${voucher.payment_date || new Date().toISOString().split('T')[0]}</div></div>
      <div class="info-cell"><div class="info-label">💳 طريقة الدفع</div><div class="info-value">${voucher.payment_method}</div></div>
    ...
    <div class="row-full">
      ...
      <div class="row-value agent-name">${agentName}</div>
    </div>
    <div class="amount-box">
      ...
      <div class="amount-center">
        <div class="amount-number">${amount.toLocaleString('ar-LY')}</div>
      </div>
    ...
    <div class="row-full">
      ...
      <div class="row-value">${voucher.notes || `تسديد دفعة كشف حساب تأمين – ${agentName}`}</div>
    </div>
    <div class="signatures">
      <div class="sig-col"><div class="sig-space"></div><div class="sig-line"></div><div class="sig-label">توقيع المستلم</div></div>
      <div class="sig-col"><div class="stamp-circle">الختم<br/>الرسمي</div><div class="sig-label">اعتماد الخزينة</div></div>
      <div class="sig-col"><div class="sig-space"></div><div class="sig-line"></div><div class="sig-label">المحاسب المسؤول</div></div>
    </div>
    <div class="footer">
  ...
  <button class="print-btn" onclick="window.print(); window.onafterprint = () => window.close();">🖨️ طباعة الإيصال</button>
  <script>
