...
export default function ProfessionalLiabilityInsuranceList({ isArchive = false }: { isArchive?: boolean } = {}) {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      let allDocs: ProfessionalLiabilityInsuranceDocument[] = [...documents];
      ...
      try {
        ...
        let total = 0;
        let lastPage = 1;
        const fetchedDocs: ProfessionalLiabilityInsuranceDocument[] = [];
        ...
        do {
          ...
          const rows: ProfessionalLiabilityInsuranceDocument[] = json.data || [];
      ...
      await generatePremiumExcel({
        title: 'شركة المدار الليبي للتأمين - تقرير تأمين المسؤولية المهنية (الطبية)',
        subtitle: `إجمالي الوثائق المصدرة: ${data.length} - تاريخ الاستخراج: ${new Date().toLocaleDateString('ar-LY')}`,
        columns,
        data,
        fileName: 'تقرير_المسؤولية_المهنية',
        qrData: `تأمين مسؤولية مهنية - شركة المدار الليبي\nعدد الوثائق: ${data.length}\nبواسطة: ${currentUser.name || 'النظام'}`
      });
  ...
  return (
    <section className="users-management">
                    ...
                    return (
                      <tr key={doc.id}>
                        ...
                        <div className="action-buttons">
                          <button
                            onClick={() => {
                              window.open(`${API_BASE_URL}/professional-liability-insurance-documents/${doc.id}/print?t=${Date.now()}`, '_blank');
                            }}
      ...
      <InsuranceTermsModal
        ...
        insuranceTypeKey="liability"
        insuranceTypeName="تأمين المسؤولية المهنية"
      />
          {cancellationDoc && (
        <CancelDocumentModal
          ...
          documentType="تأمين مسؤولية مهنية"
