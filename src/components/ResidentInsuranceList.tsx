...
export default function ResidentInsuranceList({ isArchive = false }: { isArchive?: boolean } = {}) {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/resident-insurance-documents?${params.toString()}`;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/resident-insurance-documents/${showDeleteModal.id}`, {
        method: 'DELETE',
        headers
      });
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      let allDocs: ResidentInsuranceDocument[] = [...documents];
      ...
      try {
        ...
        let total = 0;
        let lastPage = 1;
        const fetchedDocs: ResidentInsuranceDocument[] = [];
        ...
        do {
          ...
          const rows: ResidentInsuranceDocument[] = json.data || [];
  ...
  return (
    <section className="users-management">
                      ...
                      <tr key={doc.id}>
                        ...
                        <td>
                          <div className="action-buttons">
                            <button
                              onClick={() => {
                                window.open(`${API_BASE_URL}/resident-insurance-documents/${doc.id}/print?t=${Date.now()}`, '_blank');
                              }}
                        ...
                        <div className="user-mobile-actions">
                          <button
                            onClick={() => {
                              ...
                              iframe.src = `${API_BASE_URL}/resident-insurance-documents/${doc.id}/print`;
      ...
      <InsuranceTermsModal
        ...
        insuranceTypeKey="resident"
        insuranceTypeName="تأمين الوافدين والإقامة"
      />
          {cancellationDoc && (
        <CancelDocumentModal
