...
const ViewSchoolStudentInsurance: React.FC = () => {
  ...
  const fetchDocument = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/school-student-insurance/${id}`);
  ...
  if (loading) return (
    <section className="users-management">
      <div className="users-breadcrumb"><span>تأمين حماية طلاب المدارس / عرض الوثيقة</span></div>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', background: 'var(--panel)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '42px', height: '42px', background: 'linear-gradient(135deg,#1e40af,#0284c7)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fa-solid fa-school" style={{ color: '#fff', fontSize: '1.1rem' }}></i>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: 'var(--muted)', fontWeight: '600' }}>تأمين حماية طلاب المدارس</div>
        ...
        <div style={{ padding: '16px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '16px' }}>
            {[
              ...
              { icon: 'fa-graduation-cap', label: 'السنة الدراسية', value: document.grade || '-', color: '#f59e0b' },
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            <SectionCard title="بيانات الوثيقة" icon="fa-file-contract">
              <Row label="رقم الوثيقة" value={document.policy_number} />
              <Row label="تاريخ الإصدار" value={fmtDateTime(document.created_at)} />
              <Row label="الحالة" value={
                <span style={{ color: document.status === 'active' ? '#10b981' : '#f59e0b', fontWeight: '700' }}>
                  {document.status === 'active' ? '✓ نشطة' : document.status}
                </span>
              } />
            ...
            <SectionCard title="بيانات الطالب والمدرسة" icon="fa-user-graduate">
              <Row label="اسم الطالب" value={document.student_name || '-'} />
              <Row label="اسم المدرسة" value={document.school_name || '-'} />
              <Row label="السنة الدراسية" value={document.grade || '-'} />
