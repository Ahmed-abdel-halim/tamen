...
export default function EditProfessionalLiabilityInsurance() {
  ...
  const [showDeleteProfessionModal, setShowDeleteProfessionModal] = useState<{ id: number; name: string } | null>(null);
  ...
  const fetchDocument = async () => {
    try {
      ...
      const data: ProfessionalLiabilityInsuranceDocument = await res.json();
      
      // ملء النموذج بالبيانات
      const issueDate = data.issue_date ? new Date(data.issue_date).toISOString().split('T')[0] : '';
      // تنسيق end_date كـ DD/MM/YYYY
      ...
      if (data.end_date) {
        ...
        endDateFormatted = `${day}/${month}/${year}`;
      }
      setFormData({
        issue_date: issueDate,
        end_date: endDateFormatted,
        contract_relation: data.contract_relation || 'نفسه',
        ...
        premium: data.premium ? (typeof data.premium === 'number' ? data.premium.toFixed(3) : parseFloat(data.premium).toFixed(3)) : '210.000',
        tax: data.tax ? (typeof data.tax === 'number' ? data.tax.toFixed(3) : parseFloat(data.tax).toFixed(3)) : '2.500',
        stamp: data.stamp ? (typeof data.stamp === 'number' ? data.stamp.toFixed(3) : parseFloat(data.stamp).toFixed(3)) : '0.500',
        issue_fees: data.issue_fees ? (typeof data.issue_fees === 'number' ? data.issue_fees.toFixed(3) : parseFloat(data.issue_fees).toFixed(3)) : '10.000',
        supervision_fees: data.supervision_fees ? (typeof data.supervision_fees === 'number' ? data.supervision_fees.toFixed(3) : parseFloat(data.supervision_fees).toFixed(3)) : '1.050',
        total: data.total ? (typeof data.total === 'number' ? data.total.toFixed(3) : parseFloat(data.total).toFixed(3)) : '224.050',
      });
    } catch (error: any) {
  ...
  // إضافة مهنة جديدة
  const handleAddProfession = async (e: React.MouseEvent | React.KeyboardEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    try {
      ...
      if (res.ok) {
        const newProfession = await res.json();
        setProfessions([...professions, newProfession]);
        setFormData({ ...formData, profession: newProfession.name });
  ...
  // حذف مهنة
  const handleDeleteProfessionClick = (e: React.MouseEvent, professionId: number, professionName: string) => {
    e.preventDefault();
    e.stopPropagation();
    setShowDeleteProfessionModal({ id: professionId, name: professionName });
  };

  const handleDeleteProfession = async () => {
    if (!showDeleteProfessionModal) return;
    ...
    try {
      ...
      if (res.ok) {
        setProfessions(professions.filter(p => p.id !== showDeleteProfessionModal.id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/professional-liability-insurance-documents/${id}`, {
        ...
        headers,
        body: JSON.stringify({
          contract_relation: formData.contract_relation,
  ...
  return (
    <section className="users-management">
      <div className="users-breadcrumb">
        <span>تأمين المسؤولية المهنية (الطبية) / تعديل وثيقة</span>
      </div>
        ...
        {loading ? (
          ...
          <div className="form-page-header">
            <h2 className="form-page-title">
              تعديل وثيقة تأمين مسؤولية مهنية (طبية)
            </h2>
            <button 
            ...
            <div className="form-sections-container">
              <div className="form-section">
                ...
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    value={formData.issue_date ? new Date(formData.issue_date).toLocaleDateString('ar-LY') : ''}
                    disabled
                    style={{ background: '#f3f4f6', color: '#6b7280' }}
                  />
                </div>
              ...
              <div className="form-section">
                ...
                <div className="form-group">
                  <label htmlFor="contract_relation">صلة التعاقد في المؤمن له <span className="required">*</span></label>
                  <select
                    id="contract_relation"
                    value={formData.contract_relation}
                    onChange={(e) => setFormData({ ...formData, contract_relation: e.target.value })}
                  >
                ...
                <div className="form-group" ref={professionDropdownRef} style={{ position: 'relative' }}>
                  ...
                  {showProfessionDropdown && (
                    ...
                    >
                      ...
                      <div style={{ maxHeight: '250px', overflowY: 'auto' }}>
                        {filteredProfessions.length === 0 && !showAddProfession ? (
                          <div style={{ padding: '12px', textAlign: 'center', color: '#9ca3af' }}>
                            لا توجد نتائج
                          </div>
                        ) : (
                          filteredProfessions.map((prof) => (
                            <div
                              key={prof.id}
                              onClick={() => {
                                setFormData({ ...formData, profession: prof.name });
                                setShowProfessionDropdown(false);
                                setProfessionSearch('');
                              }}
                              style={{
                                ...
                                cursor: 'pointer',
                                borderBottom: '1px solid #f3f4f6',
                                backgroundColor: formData.profession === prof.name ? '#f3f4f6' : 'transparent',
                                display: 'flex',
                                justifyContent: 'space-between',
                                alignItems: 'center',
                              }}
                              onMouseEnter={(e) => {
                                if (formData.profession !== prof.name) {
                                  e.currentTarget.style.backgroundColor = '#f9fafb';
                                }
                              }}
                              onMouseLeave={(e) => {
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={(e) => handleDeleteProfessionClick(e, prof.id, prof.name)}
                                style={{
                                  padding: '4px 8px',
                                  background: '#ef4444',
                                  color: '#fff',
      ...
      {showDeleteProfessionModal && (
        <div className="modal-overlay" onClick={() => !deletingProfession && setShowDeleteProfessionModal(null)}>
          <div className="modal-content delete-confirm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="delete-confirm-message">
              هل أنت متأكد من حذف المهنة <strong>{showDeleteProfessionModal.name}</strong>؟
              <br />
            ...
            <div className="delete-confirm-actions">
              <button
                className="btn-cancel"
                onClick={() => setShowDeleteProfessionModal(null)}
              ...
              <button
                ...
                disabled={deletingProfession}
              >
                {deletingProfession ? 'جاري الحذف...' : 'حذف'}
              </button>
            </div>
          </div>
        </div>
