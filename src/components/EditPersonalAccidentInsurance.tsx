...
export default function EditPersonalAccidentInsurance() {
  ...
  const fetchDocument = async () => {
    try {
      ...
      const data: PersonalAccidentInsuranceDocument = await res.json();
      ...
      setFormData({
        ...
        claim_authorized_name: data.claim_authorized_name || '',
        premium: data.premium ? (typeof data.premium === 'number' ? data.premium.toFixed(3) : parseFloat(String(data.premium)).toFixed(3)) : '200.000',
        tax: data.tax ? (typeof data.tax === 'number' ? data.tax.toFixed(3) : parseFloat(String(data.tax)).toFixed(3)) : '2.500',
        stamp: data.stamp ? (typeof data.stamp === 'number' ? data.stamp.toFixed(3) : parseFloat(String(data.stamp)).toFixed(3)) : '0.500',
        issue_fees: data.issue_fees ? (typeof data.issue_fees === 'number' ? data.issue_fees.toFixed(3) : parseFloat(String(data.issue_fees)).toFixed(3)) : '10.000',
        supervision_fees: data.supervision_fees ? (typeof data.supervision_fees === 'number' ? data.supervision_fees.toFixed(3) : parseFloat(String(data.supervision_fees)).toFixed(3)) : '1.050',
  ...
  return (
    <section className="users-management">
      <div className="users-breadcrumb">
        <span>تأمين الحوادث الشخصية / تعديل وثيقة</span>
      </div>
