...
type PersonalAccidentInsuranceDocument = {
  ...
  name: string; birth_date?: string; age?: number; phone?: string; whatsapp_number?: string;
  id_proof?: string; address?: string; workplace?: string; gender?: string; nationality?: string;
  profession?: string; claim_authorized_name?: string;
...
export default function ViewPersonalAccidentInsurance() {
  ...
  const fetchDocument = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/personal-accident-insurance-documents/${id}`, { headers: { 'Accept': 'application/json' } });
      if (!res.ok) { if (res.status === 404) { showToast('الوثيقة غير موجودة', 'error'); setTimeout(() => navigate('/personal-accident-insurance-documents'), 2000); return; } throw new Error(`HTTP error! status: ${res.status}`); }
  ...
  const handlePrint = () => {
    ...
    iframe.src = `${API_BASE_URL}/personal-accident-insurance-documents/${id}/print`;
  ...
  return (
    <section className="users-management">
      <div className="users-breadcrumb"><span>تأمين الحوادث الشخصية / عرض الوثيقة</span></div>
      <div className="users-card" style={{ padding: '0' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '40px' }}>
            ...
            <button onClick={() => navigate('/personal-accident-insurance-documents')} className="btn-submit">العودة</button>
          </div>
        ) : (
          <>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', background: 'var(--panel)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '42px', height: '42px', background: 'linear-gradient(135deg,#1e40af,#0284c7)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="fa-solid fa-heart-pulse" style={{ color: '#fff', fontSize: '1.1rem' }}></i>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--muted)', fontWeight: '600' }}>تأمين الحوادث الشخصية</div>
                  <div style={{ fontSize: '1.1rem', fontWeight: '800', color: 'var(--text)' }}>تفاصيل: {document.insurance_number}</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                {[
                  { label: 'العودة', icon: 'fa-arrow-right', bg: 'var(--panel)', border: 'var(--border)', color: 'var(--text)', onClick: () => navigate('/personal-accident-insurance-documents') },
                  { label: 'طباعة الوثيقة', icon: 'fa-print', bg: '#0f766e', border: '#0f766e', color: '#fff', onClick: handlePrint },
                  { label: 'تعديل', icon: 'fa-pencil', bg: '#2563eb', border: '#2563eb', color: '#fff', onClick: () => navigate(`/personal-accident-insurance-documents/${id}/edit`) },
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
                ...
                <SectionCard title="معلومات المؤمن له" icon="fa-user-shield">
                  <Row label="الاسم" value={document.name} />
                  ...
                  <Row label="إثبات شخصي" value={document.id_proof || '-'} />
                  ...
                  {document.claim_authorized_name && <Row label="اسم الموكل للمطالبات" value={document.claim_authorized_name} />}
                </SectionCard>
              </div>
        ...
        <CancelDocumentModal
          ...
          insuredName={(document as any).name || (document as any).insured_name || ''}
          documentType="تأمين حوادث شخصية"
