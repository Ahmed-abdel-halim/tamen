...
type TravelInsurancePassenger = {
  id: number; is_main_passenger: boolean; relationship?: string;
  name_ar: string; name_en: string; phone?: string; passport_number?: string;
  address?: string; birth_date?: string; age?: number; gender?: string; nationality?: string;
};
type TravelInsuranceDocument = {
  id: number; insurance_number: string; insurance_type: string;
  issue_date: string; start_date: string; end_date: string; duration?: string;
  geographic_area?: string; residence_type?: string; residence_duration?: number | string | null;
  premium: number; family_members_premium: number; stamp: number;
  issue_fees: number; supervision_fees: number; total: number;
  whatsapp_number?: string; passengers?: TravelInsurancePassenger[];
};
...
export default function ViewTravelInsurance() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [document, setDocument] = useState<TravelInsuranceDocument | null>(null);
  ...
  const fetchDocument = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_URL}/travel-insurance-documents/${id}`, { headers: { 'Accept': 'application/json' } });
      if (!res.ok) {
        if (res.status === 404) { showToast('الوثيقة غير موجودة', 'error'); setTimeout(() => navigate('/travel-insurance-documents'), 2000); return; }
  ...
  const handlePrint = () => {
    ...
    iframe.onload = () => {
      setTimeout(() => {
        ...
        setTimeout(() => { if (window.document.body.contains(iframe)) window.document.body.removeChild(iframe); }, 300);
      }, 100);
    };
  };
  ...
  if (loading) return (
    <section className="users-management">
      <div className="users-breadcrumb"><span>وثائق تأمين المسافرين / عرض وثيقة</span></div>
  ...
  if (!document) return (
    <section className="users-management">
      ...
      <div className="users-card" style={{ textAlign: 'center', padding: '40px' }}>
        ...
        <button onClick={() => navigate('/travel-insurance-documents')} className="btn-submit">العودة</button>
      </div>
    </section>
  );
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', background: 'var(--panel)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '42px', height: '42px', background: 'linear-gradient(135deg,#1e40af,#0284c7)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fa-solid fa-plane" style={{ color: '#fff', fontSize: '1.1rem' }}></i>
            </div>
            <div>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            {[
              { label: 'العودة', icon: 'fa-arrow-right', bg: 'var(--panel)', border: 'var(--border)', color: 'var(--text)', onClick: () => navigate('/travel-insurance-documents') },
              { label: 'طباعة الوثيقة', icon: 'fa-print', bg: '#0f766e', border: '#0f766e', color: '#fff', onClick: handlePrint },
              { label: 'تعديل', icon: 'fa-pencil', bg: '#2563eb', border: '#2563eb', color: '#fff', onClick: () => navigate(`/travel-insurance-documents/${id}/edit`) },
            ].map((btn, i) => (
        ...
        <div style={{ padding: '16px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            <SectionCard title="معلومات الوثيقة" icon="fa-file-contract">
              ...
              {document.residence_type && <Row label="نوع الإقامة" value={document.residence_type} />}
              {document.residence_duration != null && document.residence_duration !== '' && <Row label="مدة الإقامة" value={`${document.residence_duration} يوم`} />}
            </SectionCard>
            ...
            <SectionCard title="بيانات المسافر الرئيسي" icon="fa-user-shield">
              ...
              ) : <div style={{ padding: '16px', color: 'var(--muted)', textAlign: 'center' }}>لا توجد بيانات مسافر</div>}
            </SectionCard>
          </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1px', background: 'var(--border)' }}>
                {familyMembers.map((m, i) => (
                  <div key={m.id || i} style={{ background: 'var(--panel)', padding: '14px 16px' }}>
                    ...
                    {[
                      { label: 'الاسم بالعربي', val: m.name_ar },
                      { label: 'الاسم بالإنجليزي', val: m.name_en },
                      { label: 'رقم الجواز', val: m.passport_number },
                      { label: 'تاريخ الميلاد', val: m.birth_date ? fmtDate(m.birth_date) : null },
                      { label: 'العمر', val: m.age },
                      { label: 'الجنس', val: m.gender },
