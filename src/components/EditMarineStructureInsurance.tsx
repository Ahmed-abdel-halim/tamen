...
export default function EditMarineStructureInsurance() {
  ...
  const fetchDocument = async (documentId: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('فشل جلب بيانات الوثيقة');
      }
      const data = await res.json();
      
      // تحويل البيانات من API إلى formData
      ...
      setFormData({
        ...
        duration: data.duration || 'سنة (365 يوم)',
        ...
        port: data.port || '',
        registration_authority_id: data.registration_authority_id?.toString() || '',
        plate_number: data.plate_number || '',
        hull_number: data.hull_number || '',
        manufacturing_material: data.manufacturing_material || '',
        ...
        passenger_count: data.passenger_count?.toString() || '',
        ...
        stamp: data.stamp?.toString() || '0.500',
        issue_fees: data.issue_fees?.toString() || '2.000',
        supervision_fees: data.supervision_fees?.toString() || '0.500',
        total: data.total?.toString() || '',
      });

      // تحميل بيانات المحركات
      if (data.engines && Array.isArray(data.engines)) {
        const mainEngine = data.engines.find((e: any) => e.engine_type === 'main');
        const auxiliaryEngine = data.engines.find((e: any) => e.engine_type === 'auxiliary');
        
        const enginesData: Engine[] = [];
        
        if (mainEngine) {
          enginesData.push({
            id: 'engine-main',
            engine_type: 'main',
            engine_model: mainEngine.engine_model || '',
            fuel_type: mainEngine.fuel_type || '',
            engine_number: mainEngine.engine_number || '',
            manufacturing_country: mainEngine.manufacturing_country || '',
            horsepower: mainEngine.horsepower?.toString() || '',
            installation_date: mainEngine.installation_date ? mainEngine.installation_date.split('T')[0] : '',
            cylinders_count: mainEngine.cylinders_count?.toString() || '',
            installation_type: mainEngine.installation_type || '',
          });
        } else {
          enginesData.push({
            id: 'engine-main',
        ...
        if (auxiliaryEngine) {
          enginesData.push({
            id: 'engine-aux',
            engine_type: 'auxiliary',
            engine_model: auxiliaryEngine.engine_model || '',
            fuel_type: auxiliaryEngine.fuel_type || '',
            engine_number: auxiliaryEngine.engine_number || '',
            manufacturing_country: auxiliaryEngine.manufacturing_country || '',
            horsepower: auxiliaryEngine.horsepower?.toString() || '',
            installation_date: auxiliaryEngine.installation_date ? auxiliaryEngine.installation_date.split('T')[0] : '',
            cylinders_count: auxiliaryEngine.cylinders_count?.toString() || '',
            installation_type: auxiliaryEngine.installation_type || '',
          });
        } else {
          enginesData.push({
            id: 'engine-aux',
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/marine-structure-insurance-documents/${id}`, {
        ...
        body: JSON.stringify({
          ...
          hull_number: formData.hull_number || null,
          manufacturing_material: formData.manufacturing_material || null,
          length: formData.length ? parseFloat(formData.length) : null,
          width: formData.width ? parseFloat(formData.width) : null,
          depth: formData.depth ? parseFloat(formData.depth) : null,
          manufacturing_year: formData.manufacturing_year ? parseInt(formData.manufacturing_year) : null,
          manufacturing_country: formData.manufacturing_country || null,
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({ message: 'حدث خطأ أثناء تحديث الوثيقة' }));
        if (errorData.errors) {
          setFormErrors(errorData.errors);
        }
        throw new Error(errorData.message || 'حدث خطأ أثناء تحديث الوثيقة');
      }
  ...
  return (
    <section className="users-management">
      <div className="users-breadcrumb">
        <span>وثائق تأمين الهياكل البحرية / تعديل وثيقة</span>
      </div>
      ...
      <div className="users-card">
        <div className="form-page-container">
          <div className="form-page-header">
            <h2 className="form-page-title">تعديل وثيقة تأمين هياكل بحرية</h2>
            <button 
